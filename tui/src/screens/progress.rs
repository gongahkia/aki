...
impl ProgressScreen {
    pub fn new() -> Self {
        Self {
            phase: Phase::Loading(Spinner::new("Loading progress...")),
            pending: None,
        }
    }
}
...
impl Screen for ProgressScreen {
    ...
    fn on_enter(&mut self, ctx: &mut AppContext) {
        self.phase = Phase::Loading(Spinner::new("Loading progress..."));
        let api = ctx.api_url.clone();
        self.pending = Some(tokio::spawn(async move {
            let client = crate::api::client::ApiClient::new(&api);
            client.get_progress_summary().await
        }));
    }
            ...
            match result {
                Ok(Ok(summary)) => {
                    self.phase = Phase::Display(
                        serde_json::to_string_pretty(&summary).unwrap_or_else(|_| "No data".into()),
                    );
                }
                Ok(Err(e)) => {
                    let msg = format!("{}", e);
